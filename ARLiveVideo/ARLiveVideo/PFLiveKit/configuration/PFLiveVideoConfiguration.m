...
@implementation PFLiveVideoConfiguration
+ (instancetype)defaultConfiguration {
    PFLiveVideoConfiguration *configuration = [PFLiveVideoConfiguration defaultConfigurationForQuality:PFLiveVideoQuality_Default];
    return configuration;
}

+ (instancetype)defaultConfigurationForQuality:(PFLiveVideoQuality)videoQuality {
    PFLiveVideoConfiguration *configuration = [PFLiveVideoConfiguration defaultConfigurationForQuality:videoQuality outputImageOrientation:UIInterfaceOrientationPortrait];
    return configuration;
}
...
+ (instancetype)defaultConfigurationForQuality:(PFLiveVideoQuality)videoQuality outputImageOrientation:(UIInterfaceOrientation)outputImageOrientation {
    PFLiveVideoConfiguration *configuration = [PFLiveVideoConfiguration new];
    switch (videoQuality) {
    case PFLiveVideoQuality_Low1:{
        configuration.sessionPreset = PFCaptureSessionPreset360x640;
        configuration.videoFrameRate = 15;  // 帧率
    ...
    case PFLiveVideoQuality_Low3: {
        ...
        break;
    case PFLiveVideoQuality_Medium1:{
        configuration.sessionPreset = PFCaptureSessionPreset540x960;
        ...
        break;
    case PFLiveVideoQuality_Medium2:{
        configuration.sessionPreset = PFCaptureSessionPreset540x960;
        configuration.videoFrameRate = 24;
        configuration.videoMaxFrameRate = 24;
        configuration.videoMinFrameRate = 12;
        ...
        break;
    case PFLiveVideoQuality_Medium3:{
        configuration.sessionPreset = PFCaptureSessionPreset540x960;
        configuration.videoFrameRate = 30;
        configuration.videoMaxFrameRate = 30;
        configuration.videoMinFrameRate = 15;
        ...
        break;
    case PFLiveVideoQuality_High1:{
        configuration.sessionPreset = PFCaptureSessionPreset720x1280;
        configuration.videoFrameRate = 15;
        configuration.videoMaxFrameRate = 15;
        configuration.videoMinFrameRate = 10;
        configuration.videoBitRate = 1000 * 1000;
...
- (NSString *)avSessionPreset {
    ...
    case PFCaptureSessionPreset360x640:{
        avSessionPreset = AVCaptureSessionPreset640x480;
    }
        break;
    case PFCaptureSessionPreset540x960:{
        avSessionPreset = AVCaptureSessionPresetiFrame960x540;
    }
        break;
    case PFCaptureSessionPreset720x1280:{
...
- (void)setSessionPreset:(PFLiveVideoSessionPreset)sessionPreset{
    _sessionPreset = sessionPreset;
    _sessionPreset = [self supportSessionPreset:sessionPreset];
}

#pragma mark -- Custom Method
- (PFLiveVideoSessionPreset)supportSessionPreset:(PFLiveVideoSessionPreset)sessionPreset {
    ...
    if (![session canSetSessionPreset:self.avSessionPreset]) {
        if (sessionPreset == PFCaptureSessionPreset720x1280) {
            sessionPreset = PFCaptureSessionPreset540x960;
            if (![session canSetSessionPreset:self.avSessionPreset]) {
                sessionPreset = PFCaptureSessionPreset360x640;
            }
        } else if (sessionPreset == PFCaptureSessionPreset540x960) {
            sessionPreset = PFCaptureSessionPreset360x640;
        }
    }
...
- (BOOL)isEqual:(id)other {
    if (other == self) {
        ...
        PFLiveVideoConfiguration *object = other;
...
- (id)copyWithZone:(nullable NSZone *)zone {
    PFLiveVideoConfiguration *other = [self.class defaultConfiguration];
    return other;
}

- (NSString *)description {
    NSMutableString *desc = @"".mutableCopy;
    [desc appendFormat:@"<PFLiveVideoConfiguration: %p>", self];
